...
public class StopDAO {
	...
	public Stop getById(int id) {
		...
		Cursor c = bdd
				.query(DataBaseHelper.TABLE_STOP,
						DataBaseHelper.STOP_CURSOR_QUERY, where, null, null,
						null, null);
	...
	public List<Stop> getAll() {
		Cursor c = bdd.query(DataBaseHelper.TABLE_STOP,
				DataBaseHelper.STOP_CURSOR_QUERY, null, null, null, null, null);
		return cursorToStops(c);
	}
	...
	public List<Stop> cursorToStops(Cursor c) {
		...
		for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
			...
			stop.setLon(c.getDouble(c.getColumnIndex(DataBaseHelper.STOP_LON)));
			stop.setPosition(c.getInt(c.getColumnIndex(DataBaseHelper.STOP_NUM)));
			stop.setIdLine(c.getString(c
					.getColumnIndex(DataBaseHelper.STOP_IDLINE)));
			lines.add(stop);
		}
